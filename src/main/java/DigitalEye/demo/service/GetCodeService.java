...
import static DigitalEye.demo.service.api.OpenApiService.*;
...
public class GetCodeService {
    ...
    public static List<String> getTrainTypesCode() {
        ...
        for (int i = 9; i < itemsArray.length(); i++) {
            JSONObject item = itemsArray.getJSONObject(i);
            int typeCode = item.getInt("vehiclekndid");
            String trainTypeCode = String.valueOf(typeCode);
            trainTypesCode.add(trainTypeCode);
        }
